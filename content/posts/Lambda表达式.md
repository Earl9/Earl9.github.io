---
title: "Lambda表达式"
date: 2021-11-08T22:57:16+08:00
draft: false
slug: ff79dbf8
tags:
  - Java
...
学会理解和写 Lambda 表达式的三步:

**1. 确认 Lambda 表达式的类型**:

- 能用 Lambda 表达式来表示的类型，必须是一个**函数式接口**，而函数式接口，就是只有一个抽象方法的接口。

**2. 找到要实现的方法**:

- Lambda 表达式就是实现一个方法，什么方法呢？就是刚刚那些函数式接口中的**抽象方法**

**3. 实现这个方法**:

- Lambda 表达式就是要实现这个抽象方法，如果不用 Lambda 表达式，你一定知道用**匿名类**如何去实现

Lambda 表达式的类型就是**函数式接口**，要实现的方法就是函数式接口里那个唯一的**抽象方法**，实现这个方法的方式就是**参数块 + 小箭头 + 方法体**，其中参数块和方法体都可以一定程度上简化它的写法。
